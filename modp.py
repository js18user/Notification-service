...
try:
    ...
    async def get_all_distributions(request: Request,
                                    db=Depends(conn.connection), ):
        return await json_response(request, await seek_json(db))
    ...
    ): return await json_response(request, await seek_messages(db, id_distribution, ))
    ...
    async def select_messages_status(
            ...
            id_distribution: int = Query(ge=0, ),
            status: str = Query(),
    ): return await json_response(request, await seek_status(db, id_distribution, status))
